...
def parse_args(args):
    ...
    parser.add_argument("--save_path", default="./pixdlm_model", type=str, required=True)
    parser.add_argument("--seg_token_num", default=3, type=int)
    parser.add_argument("--image_feature_scale_num", default=3, type=int)
...
def main(args):
    ...
    model.get_model().initialize_pixdlm_modules(model.get_model().config)
    ...
    for k, v in model.state_dict().items():
        if "vision_tower" not in k:
            state_dict[k] = v
    model.save_pretrained(args.save_path, state_dict=state_dict,max_shard_size=1000000000000000)
